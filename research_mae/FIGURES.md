...
## Fig 4 — 隐向量老化流形（学习老化轴 + 残差 PC）

**文件**：`fig4_latent_manifold.png`（1×3：D1 / D2 / D3）

![Fig 4](figures/fig4_latent_manifold.png)

### 画的是什么

将 MAE 编码器输出的 **32 维隐向量 z** 投影到二维**可解释老化流形**（**不是 t-SNE**）：

| 维度 | 含义 |
|------|------|
| **Dim 1** | `aging_head(z)` — MAE 训练时监督的**学习老化轴**（Sigmoid，∈ [0,1]） |
| **Dim 2** | 从 z 中扣除 aging 线性分量后的**残差 PCA 第一主成分** |

颜色 = **寿命比率** `cycle / cycle_max_cell`（电芯内归一化，跨电芯可比）。

子图标题中的 **Spearman ρ** = Dim 1 与寿命比率的 Spearman 相关系数（取正方向）。

### 子采样与显示

- 每数据集约 **3000** 点（从全量 ~2.2万/2.8万/0.86万 均匀抽样），**不是数据缺失**
- 坐标轴按 **0.5%–99.5% 分位裁剪**，避免个别 aging 离群点把主体云团压成一角
...
- Dim 1 从左到右、颜色从紫到黄 → 老化进程推进，流形应呈带状/弧形结构
- ρ 越接近 1，说明学习老化轴与寿命进程越单调一致
- Dim 2 展示**去老化后**的个体差异（材料、工况、噪声），不用于报 Spearman

### 实现

`thesis_figures.py` → `fig4_latent_manifold()`；训练侧见 `models.py`（aging head）、`train.py`（`_aging_targets`）。

---

## Fig 11 — 单电芯轨迹 + 全数据集老化轴投影

**文件**：`fig11_manifold_trajectory_combo.png`（2×3：D1 / D2 / D3）

![Fig 11](figures/fig11_manifold_trajectory_combo.png)
...
| 行 | 方法 | 颜色 | Spearman |
|----|------|------|----------|
| **(a) 单电芯** | 长寿命电芯隐向量 **t-SNE**（≤800 点） | 圈数 | t-SNE Dim 1 vs 圈数 |
| **(b) 全数据** | **学习老化轴 + 残差 PC**（同 Fig 4） | 寿命比率 | Dim 1 vs 寿命比率 |

灰色折线连接 (a) 中按圈数排序的相邻点，突出单电芯内连续演变。

### 怎么读

- **(a)**：应看到紫→黄沿轨迹方向渐变，体现单电芯老化是低维连续过程
- **(b)**：验证跨电芯时老化轴仍与寿命比率单调一致（ρ 与 Fig 4 相同）
- D1/D2 的 (a) 往往比 (b) 更「干净」，说明跨电芯个体差异主要体现在残差维（Dim 2）

### 实现

`thesis_figures.py` → `fig11_manifold_trajectory_combo()`；与 Fig 4 共用 `_population_aging_embedding()`。
